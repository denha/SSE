...
def encrypt_aes_file_sync(filename, key, output_folder, cipher_file_name):
    chunk_size = 128 * 1024
    cipher_file_name = cipher_file_name + '.enc'
    output_filename = os.path.join(output_folder, os.path.basename(cipher_file_name))
    iv = os.urandom(16)
    ...
    with io.BytesIO(filename.encode('utf-8')) as input_file, open(output_filename, 'wb') as output_file:
        output_file.write(iv)
        while True:
            chunk = input_file.read(chunk_size)
            if len(chunk) == 0:
